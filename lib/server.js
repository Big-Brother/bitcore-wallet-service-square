    ...
    self._getUtxosForCurrentWallet(null, function(err, utxos) {
      ...
      var info = {
       ...
       if(opts.isInstantSend===true){
        ...
        if(!validUTXO)
            return cb(new Error('InstantSend requires all Square you are sending (transaction inputs) to be held for about 15-20 minutes (minimum 6 network confirmations). Please wait before using InstantSend.'));
       }
 ...
 * @param {Object} opts
 * @param {string} [opts.network = 'livenet'] - The Square network for this transaction.
